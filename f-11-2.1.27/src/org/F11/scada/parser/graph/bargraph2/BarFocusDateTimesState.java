...
public class BarFocusDateTimesState
		implements
			State,
			TextDateTimeSymbolContainerState {
	private static Logger logger = Logger.getLogger(BarFocusDateTimesState.class);

	private BarFocus focus;
	/**
	 * 状態を表すオブジェクトを生成します。
	 */
	public BarFocusDateTimesState(String tagName, Attributes atts,
			BarFocus focus) {
		this.focus = focus;
	}
	...
	public void addTextDateTimeSymbol(TextDateTimeSymbol symbol) {
		focus.addTextDateTimeSymbol(symbol);
	}
