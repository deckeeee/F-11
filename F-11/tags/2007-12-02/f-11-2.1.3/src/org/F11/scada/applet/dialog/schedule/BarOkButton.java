 ...
 * OK ボタンクラスです
 */
class BarOkButton extends AbstractOkButton {
	private static final long serialVersionUID = 6913668124309820409L;

	BarOkButton(AbstractScheduleDialog scheduleDialog, boolean isSort, boolean isLenient) {
	...
	protected boolean isNotRegalTime(int[][] times) {
		for (int i = 0; i < times.length; i++) {
			...
			if (!isLenient && onTime > offTime) {
				return true;
			}
			if (onTime > 2400 || offTime > 2400) {
				return true;
			}
		}
		return false;
