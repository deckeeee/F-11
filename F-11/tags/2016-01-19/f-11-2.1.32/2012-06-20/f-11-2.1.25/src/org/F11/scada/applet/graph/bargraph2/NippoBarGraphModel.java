...
public class NippoBarGraphModel extends AbstractBarGraphModel {
	/** ロギングAPI */
	private static Logger logger = Logger.getLogger(NippoBarGraphModel.class);
	...
	public NippoBarGraphModel() throws RemoteException {
		super();
	}
	...
	protected BarData[][] loadLoggingData() throws RemoteException {
		...
		first.add(Calendar.DAY_OF_MONTH, -getBarCount());
	...
	private BarData[][] rebuildData(Date startDate,
		...
		for (int bar = 0; bar < getBarCount(); bar++) {
			...
			cal.add(Calendar.DAY_OF_MONTH, bar);
			for (int block = 0; block < getBlockCount(); block++) {
				result[block][bar] = null;
				DoubleList dlist = searchHour(source, cal);
				if (dlist != null) {
					...
					key.add(Calendar.HOUR_OF_DAY, -1); // 値は前時分
	...
	/*
	 * ロードデータから指定日時～１時間以内のデータを検索
	 */
	private DoubleList searchHour(SortedMap<Timestamp, DoubleList> source,
			Calendar cal) {
	...
	protected void setNowValue(BarData[][] dataMatrix) {
		...
		int block = today.get(Calendar.HOUR_OF_DAY);
		// 参照日前時から
		...
		if (start.compareTo(today) <= 0 && today.before(end)) {
			...
			if (data == null) {
				data = new BarData();
				data.setDate(today.getTime()); // 現在値は当時分
	...
	public synchronized void setReferenceDate(ReferenceDate refDate) {
		...
		refCal.set(Calendar.HOUR_OF_DAY, 1);
	...
	public synchronized void changePeriod(int offset) {
		...
		refCal.add(Calendar.DAY_OF_MONTH, offset);
		Calendar cal = new GregorianCalendar();
		cal.add(Calendar.HOUR_OF_DAY, 1); // データ日時に合わせる
		if (refCal.after(cal) || offset == 0) {
			refCal.setTime(cal.getTime());
			refCal.add(Calendar.HOUR_OF_DAY, -1); // 23時の対応
		}
		cal.setTime(firstLoggingDate);
		cal.add(Calendar.DAY_OF_MONTH, -1);
		if (refCal.before(cal)) { // 先頭データは本日範囲か
			refCal.setTime(firstLoggingDate);
			refCal.add(Calendar.HOUR_OF_DAY, -1); // 0時の対応
		}
		refCal.set(Calendar.HOUR_OF_DAY, 1);
		refCal.set(Calendar.MINUTE, 0);
		refCal.set(Calendar.SECOND, 0);
