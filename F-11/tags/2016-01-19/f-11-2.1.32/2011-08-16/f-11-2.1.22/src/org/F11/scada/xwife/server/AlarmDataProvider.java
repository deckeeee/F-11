 ...
 * 起動時にデータソースより過去データを読込再現します。以降は値変更イベントを受け取り、保持テーブルを更新します。
 * 保持テーブルは Collector オブジェクトにより参照されます。
 * 
 * @author Hideaki Maekawa <frdm@users.sourceforge.jp>
 */
public class AlarmDataProvider
		extends AbstractDataProvider
		implements Runnable, AlarmDataStore, Service {
	...
	/**
	 * コンストラクタ
	 * 各警報一覧テーブルモデルをDBより生成しデータホルダに登録し、
	 * 警報プロパイダを初期化し、JIM Managerにプロバイダ登録します。
	 * 
	 ...
	 */
	public AlarmDataProvider(StrategyFactory factory)
			throws DataProviderDoesNotSupportException, InstantiationException, IllegalAccessException, AlarmException {
		super();
	...
	private RowDataStrategy createCareerHolder(
			InitialTableFactory tableFactory,
			StrategyFactory factory,
			long now) throws DataProviderDoesNotSupportException, AlarmException {
	...
	private RowDataStrategy createHistoryHolder(
			InitialTableFactory tableFactory,
			StrategyFactory factory,
			long now)
			throws DataProviderDoesNotSupportException,
	...
	private RowDataStrategy createSummaryHolder(
			InitialTableFactory tableFactory,
			StrategyFactory factory,
			long now)
			throws DataProviderDoesNotSupportException,
	...
	private RowDataStrategy createOccurrenceHolder(
			InitialTableFactory tableFactory,
			StrategyFactory factory,
			long now)
			throws AlarmException,
	...
	private RowDataStrategy createNoncheckHolder(
			InitialTableFactory tableFactory,
			StrategyFactory factory,
			long now)
			throws AlarmException,
	...
	private DataHolder createDataHolder(long now, AlarmTableModel tableModel, String holderName) {
	...
	public void run() {
		...
		while (ct == thread) {
			//キューが空の間はwaitします。
			DataValueChangeEventKey evt = (DataValueChangeEventKey) queue.dequeue();
