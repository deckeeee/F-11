 ...
 * @author hori
 */
public class SymbolAnalog4EditableState implements State {
	...
	static {
		...
		editableClassName.put(
			...
			"org.F11.scada.applet.symbol.TextAnalog4SymbolEditable");
		editableClassName.put(
	...
	private Analog4Editable symbolAnalog4Editable;
	private Symbol symbol;

	/**
	 * 状態を表すオブジェクトを生成します。
	 */
	public SymbolAnalog4EditableState(
		String tagName,
		Attributes atts,
		...
		if ("textanalog4symboleditable".equals(tagName)) {
			createSymbol(state, createSymbol(tagName, atts, state));
			symbolAnalog4Editable.setSecondDialogName("1");
		} else if ("textpowerfactor4symboleditable".equals(tagName)) {
			createSymbol(state, createSymbol(tagName, atts, state));
			symbolAnalog4Editable.setSecondDialogName("2");
		} else {
	...
	private void createSymbol(SymbolContainerState state, Symbol symbol) {
		...
		symbolAnalog4Editable = (Analog4Editable) this.symbol;
	}
	...
	public void add(String tagName, Attributes atts, Stack stack) {
		...
		if (tagName.equals("if")) {
			...
			stack.push(new DestinationState(tagName, atts, symbolAnalog4Editable));
