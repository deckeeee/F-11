...
public class PostgreSQLAlarmListFinder implements AlarmListFinder {
	...
	public FindAlarmTable getSummaryList(
		...
		String selectHead = "SELECT i.jump_path, i.auto_jump_flag, i.auto_jump_priority, CASE WHEN s.bit_value = '1' THEN att.on_summary_color ELSE att.off_summary_color END AS alarm_color, i.point, i.provider, i.holder, s.on_date, s.off_date, p.unit, CASE WHEN att.name IS NOT NULL AND att.message_mode THEN p.name || '　' || att.name ELSE p.name || '　' END AS kikiname, m.message,pri.name AS priorityname ";
	...
	public FindAlarmTable getHistoryList(
		...
		String selectHead = "SELECT i.jump_path, i.auto_jump_flag, i.auto_jump_priority,CASE WHEN h.off_date='epoch' THEN att.on_alarm_color ELSE att.off_alarm_color END AS alarm_color,h.point, h.provider, h.holder, h.on_date,h.off_date, p.unit,CASE WHEN att.name IS NOT NULL AND att.message_mode THEN p.name || '　' || att.name ELSE p.name || '　' END AS kikiname,pri.name AS priorityname,CASE WHEN att.check_type = '0' THEN '────' WHEN att.check_type = '1' AND h.check_flag = '1' THEN '＊＊＊＊' ELSE NULL END AS histry_check ";
		StringBuffer sbBody = new StringBuffer();
		sbBody
	...
	public FindAlarmTable getCareerList(
		...
		String countHead = "SELECT COUNT(*) AS count ";
		String selectHead = "SELECT i.jump_path,CASE WHEN c.bit_value='1' THEN i.auto_jump_flag ELSE '0' END as auto_jump_flag,i.auto_jump_priority,CASE WHEN c.bit_value='1' THEN att.on_alarm_color ELSE att.off_alarm_color END as alarm_color , i.point, i.provider, i.holder, att.sound_type, CASE WHEN c.bit_value='1' THEN i.on_sound_path ELSE i.off_sound_path END AS sound_path, i.email_group_id, i.email_send_mode, c.entrydate,p.unit, CASE WHEN att.name IS NOT NULL AND att.message_mode THEN p.name || '　' || att.name ELSE p.name || '　' END AS kikiname,m.message,pri.name AS priorityname ";
		StringBuffer sbBody = new StringBuffer();
		sbBody
	...
	public List getPriorityTable() throws RemoteException {
		...
		try {
			...
			throw new RemoteException("プライオリティ名テーブル取得時にエラー発生", e);
		} finally {
			if (null != rs) {
				try {
					rs.close();
