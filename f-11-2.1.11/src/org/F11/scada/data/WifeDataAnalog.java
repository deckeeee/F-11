 ...
 */
public final class WifeDataAnalog extends Number implements WifeData, Serializable {
	...
	/**
	 * アナログデータの形式を表すタイプセーフenumクラスです。
	 * 各staticフィールドがアナログ元byte配列データの形式を表すインスタンスです。
	...
	abstract static class AnalogType implements Serializable {
		...
		static final AnalogType HEX_ZERO_HALF_WORD =
			new AnalogType(0, ByteSize.ZERO_HALF_WORD_ANALOG, "HEX_ZERO_HALF_WORD", 0, 255) {
			private static final long serialVersionUID = 3001100906618864417L;

			WifeDataAnalog valueOf(byte[] b) {
				BigInteger bi = new BigInteger(1, HEX_ZERO_HALF_WORD.byteSize.canonicalBytes(b));
		...
		static final AnalogType HEX_ONE_HALF_WORD =
			new AnalogType(0, ByteSize.ONE_HALF_WORD_ANALOG, "HEX_ONE_HALF_WORD", 0, 255) {
			private static final long serialVersionUID = -6595712322778359083L;

			WifeDataAnalog valueOf(byte[] b) {
				BigInteger bi = new BigInteger(1, HEX_ONE_HALF_WORD.byteSize.canonicalBytes(b));
		...
		static final AnalogType HEX_SINGLE_WORD =
			new AnalogType(0, ByteSize.SINGLE_WORD_ANALOG, "HEX_SINGLE_WORD", 0, 65535) {
			private static final long serialVersionUID = 7541518316060757625L;

			WifeDataAnalog valueOf(byte[] b) {
				BigInteger bi = new BigInteger(1, HEX_SINGLE_WORD.byteSize.canonicalBytes(b));
		...
		static final AnalogType HEX_DOUBLE_WORD =
			new AnalogType(0, ByteSize.DOUBLE_WORD_ANALOG, "HEX_DOUBLE_WORD", 0, 4294967295D) {
			private static final long serialVersionUID = -3026934469719700166L;

			WifeDataAnalog valueOf(byte[] b) {
				BigInteger bi = new BigInteger(1, HEX_DOUBLE_WORD.byteSize.canonicalBytes(b));
			...
			byte[] toByteArray(double value) {
				...
				if (b.length == 4) {
					...
					byte[] b2 = new byte[4];
					Arrays.fill(b2, (byte) 0x00);
					System.arraycopy(b, 0, b2, b2.length - b.length, b.length);
		...
		static final AnalogType SHX_ZERO_HALF_WORD =
			new AnalogType(
				0,
				ByteSize.ZERO_HALF_WORD_ANALOG,
				"SHX_ZERO_HALF_WORD",
				Byte.MIN_VALUE,
				Byte.MAX_VALUE) {
			private static final long serialVersionUID = 7793468773198476758L;

			WifeDataAnalog valueOf(byte[] b) {
				BigInteger bi = new BigInteger(HEX_ZERO_HALF_WORD.byteSize.canonicalBytes(b));
		...
		static final AnalogType SHX_ONE_HALF_WORD =
			new AnalogType(
				0,
				ByteSize.ONE_HALF_WORD_ANALOG,
				"SHX_ONE_HALF_WORD",
				Byte.MIN_VALUE,
				Byte.MAX_VALUE) {
			private static final long serialVersionUID = -3227886227896475778L;

			WifeDataAnalog valueOf(byte[] b) {
				BigInteger bi = new BigInteger(HEX_ONE_HALF_WORD.byteSize.canonicalBytes(b));
		...
		static final AnalogType SHX_SINGLE_WORD =
			new AnalogType(
				0,
				ByteSize.SINGLE_WORD_ANALOG,
				"SHX_SINGLE_WORD",
				Short.MIN_VALUE,
				Short.MAX_VALUE) {
			private static final long serialVersionUID = -5369305759733297768L;

			WifeDataAnalog valueOf(byte[] b) {
				BigInteger bi = new BigInteger(HEX_SINGLE_WORD.byteSize.canonicalBytes(b));
		...
		static final AnalogType SHX_DOUBLE_WORD =
			new AnalogType(
				0,
				ByteSize.DOUBLE_WORD_ANALOG,
				"SHX_DOUBLE_WORD",
				Integer.MIN_VALUE,
				Integer.MAX_VALUE) {
			private static final long serialVersionUID = 2083293025537772580L;

			WifeDataAnalog valueOf(byte[] b) {
				BigInteger bi = new BigInteger(HEX_DOUBLE_WORD.byteSize.canonicalBytes(b));
		...
		static final AnalogType BCD_ZERO_HALF_WORD =
			new AnalogType(1, ByteSize.ZERO_HALF_WORD_ANALOG, "BCD_ZERO_HALF_WORD", 0, 99) {
			private static final long serialVersionUID = -1698054779788165929L;

			WifeDataAnalog valueOf(byte[] b) {
				return WifeDataAnalog.valueOfBcdZeroHalf(
					WifeBCD.valueOf(BCD_ZERO_HALF_WORD.byteSize.canonicalBytes(b)));
			}
			...
			byte[] toByteArray(double value) {
				return BCD_ZERO_HALF_WORD.byteSize.toByteArray(WifeBCD.valueOf(value, "00"));
			}
		};
		/** アナログ元byte配列データの形式がBCDで、1バイト目使用の1/2ワード長アナログデータです */
		static final AnalogType BCD_ONE_HALF_WORD =
			new AnalogType(1, ByteSize.ONE_HALF_WORD_ANALOG, "BCD_ONE_HALF_WORD", 0, 99) {
			private static final long serialVersionUID = -376668186497495188L;

			WifeDataAnalog valueOf(byte[] b) {
				return WifeDataAnalog.valueOfBcdOneHalf(
					WifeBCD.valueOf(BCD_ONE_HALF_WORD.byteSize.canonicalBytes(b)));
			}
			...
			byte[] toByteArray(double value) {
				return BCD_ONE_HALF_WORD.byteSize.toByteArray(WifeBCD.valueOf(value));
			}
		};
		/** アナログ元byte配列データの形式がBCDで、1ワード長アナログデータです */
		static final AnalogType BCD_SINGLE_WORD =
			new AnalogType(1, ByteSize.SINGLE_WORD_ANALOG, "BCD_SINGLE_WORD", 0, 9999) {
			private static final long serialVersionUID = 7932049750401319460L;

			WifeDataAnalog valueOf(byte[] b) {
				return WifeDataAnalog.valueOfBcdSingle(
					WifeBCD.valueOf(BCD_SINGLE_WORD.byteSize.canonicalBytes(b)));
			}
			...
			byte[] toByteArray(double value) {
				return BCD_SINGLE_WORD.byteSize.toByteArray(WifeBCD.valueOf(value));
			}
		};
		/** アナログ元byte配列データの形式がBCDで、2ワード長アナログデータです */
		static final AnalogType BCD_DOUBLE_WORD =
			new AnalogType(1, ByteSize.DOUBLE_WORD_ANALOG, "BCD_DOUBLE_WORD", 0, 99999999) {
			private static final long serialVersionUID = 7328479680285566556L;

			WifeDataAnalog valueOf(byte[] b) {
				return WifeDataAnalog.valueOfBcdDouble(
					WifeBCD.valueOf(BCD_DOUBLE_WORD.byteSize.canonicalBytes(b)));
			}
			...
			byte[] toByteArray(double value) {
				return BCD_DOUBLE_WORD.byteSize.toByteArray(WifeBCD.valueOf(value, "00000000"));
			}
		};
		...
		static final AnalogType FLOAT_DOUBLE_WORD =
			new AnalogType(
				...
				Float.MIN_VALUE,
				Float.MAX_VALUE) {
			private static final long serialVersionUID = -30860737092117702L;

			WifeDataAnalog valueOf(byte[] b) {
				DataInputStream in = null;
				try {
					in = new DataInputStream(new ByteArrayInputStream(FLOAT_DOUBLE_WORD.byteSize.canonicalBytes(b)));
					return WifeDataAnalog.valueOfFloat(
						new Float(Float.intBitsToFloat(in.readInt())).doubleValue());
				} catch (IOException ex) {
					ex.printStackTrace();
					throw new IllegalArgumentException("IEEE 754 浮動小数点「単精度」ビット配列 -> float変換にしぱいした。");
				} finally {
					if (in != null) {
						try {
							in.close();
			...
			byte[] toByteArray(double value) {
				...
				try {
					...
					dos.writeInt(i);
					dos.flush();
					return FLOAT_DOUBLE_WORD.byteSize.toByteArray(os.toByteArray());
				} catch (IOException ex) {
					ex.printStackTrace();
					throw new IllegalArgumentException("IEEE 754 浮動小数点「単精度」ビット配列 -> float変換にしぱいした。");
				} finally {
			...
			void checkArgument(double value) {
				...
				if (src.isInfinite() || src.isNaN())
					throw new IllegalArgumentException("Argument value is Infinite or NaN");
			}
		};
		...
		static final AnalogType DOUBLE_FOURTH_WORD =
			new AnalogType(
				3,
				ByteSize.FOURTH_WORD_ANALOG,
				"DOUBLE_FOURTH_WORD",
				Double.MIN_VALUE,
				Double.MAX_VALUE) {
			private static final long serialVersionUID = 7765706268117247534L;

			WifeDataAnalog valueOf(byte[] b) {
				DataInputStream in = null;
				try {
					in = new DataInputStream(new ByteArrayInputStream(DOUBLE_FOURTH_WORD.byteSize.canonicalBytes(b)));
					return WifeDataAnalog.valueOfDouble(Double.longBitsToDouble(in.readLong()));
				} catch (IOException ex) {
					ex.printStackTrace();
					throw new IllegalArgumentException("IEEE 754 浮動小数点「倍精度」ビット配列 -> double変換にしぱいした。");
				} finally {
					if (in != null) {
						try {
							in.close();
			...
			byte[] toByteArray(double value) {
				...
				try {
					...
					dos.writeLong(d);
					dos.flush();
					return DOUBLE_FOURTH_WORD.byteSize.toByteArray(os.toByteArray());
				} catch (IOException ex) {
					ex.printStackTrace();
					throw new IllegalArgumentException("IEEE 754 浮動小数点「倍精度」ビット配列 -> double変換にしぱいした。");
				} finally {
		...
		/**
		 * コンストラクタ
		 * new演算子でインスタンスを生成できません。
		 * staticフィールドを参照してください。
		 */
		AnalogType(
			int type,
			ByteSize byteSize,
		...
		/**
		 * 引数を変換した値を持つWifeDataAnalogを返します。
		 * 変換の実装はそれぞれのタイプによって異なります。
		...
		/**
		 * アナログ元データのサイズのタイプセーフemunクラスです。
		 * 各staticフィールドがアナログ元データのサイズを表すインスタンスです。
		 * byte配列を引数にとりWifeDataAnalogを返す抽象クラスを実装しています。
		 * new演算子でインスタンスを生成することはできません。
		 */
		abstract static class ByteSize implements Serializable {
			...
			static final ByteSize ZERO_HALF_WORD_ANALOG = new ByteSize(1, HalfWordType.TYPE_ZERO) {
				private static final long serialVersionUID = -6385568572265636088L;

				byte[] canonicalBytes(byte[] b) {
					if (b.length != SINGLE_WORD_ANALOG.byteSize) {
						throw new IllegalArgumentException(
							"不正な長さのbyte配列が引数に使用されました。length = " + b.length);
					}
					return new byte[] { b[ZERO_HALF_WORD_ANALOG.halfWordType.valueOf()] };
				}
			...
			static final ByteSize ONE_HALF_WORD_ANALOG = new ByteSize(1, HalfWordType.TYPE_ONE) {
				private static final long serialVersionUID = 3790365881542551006L;
				...
				byte[] canonicalBytes(byte[] b) {
					...
					return new byte[] { b[ONE_HALF_WORD_ANALOG.halfWordType.valueOf()] };
				}
			...
			/** 1ワードのアナログデータです */
			static final ByteSize SINGLE_WORD_ANALOG = new ByteSize(2, HalfWordType.TYPE_ZERO) {
				private static final long serialVersionUID = -6025093020033537813L;
				...
				byte[] toByteArray(byte[] b) {
					...
					System.arraycopy(b, 0, rb, SINGLE_WORD_ANALOG.byteSize - b.length, b.length);
					return rb;
				}
			};
			/** 2ワードのアナログデータです */
			static final ByteSize DOUBLE_WORD_ANALOG = new ByteSize(4, HalfWordType.TYPE_ZERO) {
				private static final long serialVersionUID = -4525230963105073071L;
				...
				byte[] toByteArray(byte[] b) {
					...
					Arrays.fill(rb, (byte) 0x00);
					System.arraycopy(b, 0, rb, DOUBLE_WORD_ANALOG.byteSize - b.length, b.length);
					return swapBytes(rb, DOUBLE_WORD_ANALOG.byteSize / 2);
				}
			};
			/** 4ワードのアナログデータです(BACnetのみで使用？　バイト列交換は無し？) */
			static final ByteSize FOURTH_WORD_ANALOG = new ByteSize(8, HalfWordType.TYPE_ZERO) {
				private static final long serialVersionUID = -3026858847517993664L;
				...
				byte[] toByteArray(byte[] b) {
					...
					System.arraycopy(b, 0, rb, 0, FOURTH_WORD_ANALOG.byteSize);
					return rb;
				}
			};
			...
			static class HalfWordType implements Serializable {
				private static final long serialVersionUID = 2089788803489359021L;
				/** バイトタイプ */
				...
				private static int nextOrdinal = 0;
				private final int ordinal = nextOrdinal++;
				private static final HalfWordType[] VALUES = { TYPE_ZERO, TYPE_ONE };
				Object readResolve() throws ObjectStreamException {
					return VALUES[ordinal];
				}
			}
	...
	/**
	 * バイト配列をアナログ値に変換しWifeDataAnalogを返します。
	...
	/**
	 * 引数のアナログ値に変換しWifeDataAnalogを返します。
	 * アナログ値以外のフィールドは以前の値を保持します。
	 * @param value アナログ値
	 */
	...
	public boolean equals(Object obj) {
		...
		return Double.doubleToLongBits(wd.value) == Double.doubleToLongBits(value)
	...
	/**
	 * このアナログオブジェクトのワード長を返します。
	 * 但し、1/2 ワード長は1ワードとして返します。
