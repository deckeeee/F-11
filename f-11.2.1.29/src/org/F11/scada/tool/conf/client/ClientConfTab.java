...
public class ClientConfTab extends JScrollPane implements DocumentListener {
	...
	private void isTodayOrTomorrow(JPanel mainPanel) {
		JLabel label = new JLabel("ｽｹｼﾞｭｰﾙ今日、明日ﾎﾞﾀﾝを無効にする：");
		label.setToolTipText("ｽｹｼﾞｭｰﾙ今日、明日ﾎﾞﾀﾝを無効にするか設定");
		mainPanel.add(label);
		JComboBox cb = new JComboBox(new String[] { "しない", "する", });
		if ("false".equals(manager.getClientConf(
	...
	private void columnConfig(JPanel mainPanel) {
		// 警報一覧列移動設定
		mainPanel.add(new JLabel("警報一覧列移動設定："));
		JPanel panel = new JPanel(new GridLayout(1, 0));
	...
	private void setFinishPass(JPanel mainPanel) {
		JLabel label = new JLabel("クライアント終了時に認証を行う：");
		label.setToolTipText("クライアント終了時に認証を行う");
		mainPanel.add(label);
	...
	private void finishPass(JPanel mainPanel) {
		// 警報一覧列移動設定
		mainPanel.add(new JLabel("終了時パスワード："));
		JPanel panel = new JPanel(new GridLayout(1, 0));
