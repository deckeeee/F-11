		...
		if (!"".equals(soundOnHolderID)) {
			...
			if (dh != null) {
				...
				if (wd instanceof WifeDataDigital) {
					...
					logger.error("デジタル以外のデータが指定されています。 : " + soundOnHolderID);
				}
			} else {
				if (null != soundOnHolderID) {
					logger.warn(soundOnHolderID + " が登録されていません");
				}
			}
		}
	}
